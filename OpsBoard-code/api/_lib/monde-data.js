...
async function getOpenSkyStates() {
  ...
  async function fetchOpenSkyPayload(target) {
    const attempts = [
      async () => fetchJson(target, 6000),
      async () => fetchJson("https://api.allorigins.win/raw?url=" + encodeURIComponent(target), 6000),
      async () => {
        const wrapper = await fetchJson("https://api.allorigins.win/get?url=" + encodeURIComponent(target), 6000);
